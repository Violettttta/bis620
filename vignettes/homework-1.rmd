...
```{r}
file_names_all <- dir("NCT00364013")
file_names <- file_names_all[grep("sas7bdat", file_names_all)]
dt <- map(file_names, ~ read_sas(file.path("NCT00364013", .x)))
prefix <- gsub(".sas7bdat", "", file_names)
names(dt) <- prefix
```
...
length(unique(dt$respeval$SUBJID))

#All responses
table(dt$respeval$RSRESP)
```

### Create a new data set: respeval_n
### Reorder levels of responses
```{r}
respeval_n <- dt$respeval %>%
  mutate(RSRESP = 
           factor(RSRESP,
...
ar <- full_join(ar, dt$demo %>% select(SUBJID, TRT), by = "SUBJID")
ar
```
...
### Create a html to include the dat set and percent stacked barcharts

```{r fig.width=8}
library(knitr)
include_url("./html/index.html", height = "600px")
```
